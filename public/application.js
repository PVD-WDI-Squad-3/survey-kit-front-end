...
/***/ (function(module, exports, __webpack_require__) {
	...
	$(function () {
	  ...
	  $(document).on('submit', '#quiz', function (event) {
	    ...
	    //console.log(this)
	    var surveyId = $('#quiz h1').attr('id');
	    var taken = $('#taken').html();
	    //console.log(selected)
	    // console.log(taken)
	  ...
	  $(document).ready(function () {
	    ...
	    $(addButton).click(function (e) {
	      ...
	      if (x < maxFields) {
	        x++;
	        $(wrapper).append('<div class="form-group"><label for="title"></label>' + '<input class="form-control col-md-11" id="answer" type="email" placeholder="" name="author"/>' + '<a href="#" class="remove_field"><i class="fa fa-times"></a></div>');
	      }
	    });
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var createSurvey = function createSurvey(event) {
	  var data = getFormFields(this);
	  // console.log(this)
	  // console.log(data)
	  event.preventDefault();
	  appApi.newSurvey(data).then(appUi.onCreateSuccess).catch(appUi.onCreateFailure);
	};

	var viewSurveys = function viewSurveys(event) {
	  // console.log('working')
	  event.preventDefault();
	  appApi.getSurveys().then(appUi.onSurveysSuccess).catch(appUi.onSurveysFailure);
	};

	var findSurveys = function findSurveys(event) {
	  event.preventDefault();
	  appApi.findAllSurveys().then(appUi.onFindSuccess).catch(appUi.onFindFailure);
	};

	var deleteSurvey = function deleteSurvey(deleteId) {
	  appApi.deleteSurvey(deleteId).then(appUi.onDeleteSuccess).catch(appUi.onDeleteFailure);
	};

	var viewResults = function viewResults(surveyId) {
	  appApi.viewSurveyResults(surveyId).then(appUi.onViewSuccess).catch(appUi.onViewFailure);
	};

	var getNewSurvey = function getNewSurvey(survId) {
	  appApi.getSurvey(survId).then(appUi.onGetSurveySuccess).catch(appUi.onGetSurveyFailure);
	};

	var updateSurvey = function updateSurvey(surveyId, taken, data, selected) {
	  appApi.updateASurvey(surveyId, taken, data, selected).then(appUi.onUpdateSuccess).catch(appUi.onUpdateFailure);
	};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var newSurvey = function newSurvey(data) {
	  return $.ajax({
	...
	var findAllSurveys = function findAllSurveys() {
	  return $.ajax({
	    url: app.host + '/surveys',
	    method: 'GET'
	  });
	...
	var viewSurveyResults = function viewSurveyResults(surveyId) {
	  return $.ajax({
	...
	var getSurvey = function getSurvey(survId) {
	  return $.ajax({
	...
	var updateASurvey = function updateASurvey(surveyId, taken, data, selected) {
	  taken++;
	  selected++;
	  // console.log(selected)
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var onSurveysSuccess = function onSurveysSuccess(data) {
	...
	var onSurveysFailure = function onSurveysFailure(error) {
	  // console.error(error)
	};

	var onFindSuccess = function onFindSuccess(data) {
	  // console.log(data)
	...
	var onFindFailure = function onFindFailure(error) {
	  // console.error(error)
	};
	...
	var onGetSurveySuccess = function onGetSurveySuccess(data) {
	  // console.log(data)
	...
	var onGetSurveyFailure = function onGetSurveyFailure(error) {
	  // console.error(error)
	};
	...
	module.exports = {
	  ...
	  onSurveysSuccess: onSurveysSuccess,
	  onSurveysFailure: onSurveysFailure,
	  onFindSuccess: onFindSuccess,
	  onFindFailure: onFindFailure,
	  onDeleteSuccess: onDeleteSuccess,
	  onDeleteFailure: onDeleteFailure,
	  onViewSuccess: onViewSuccess,
	  onViewFailure: onViewFailure,
	  onGetSurveySuccess: onGetSurveySuccess,
	  onGetSurveyFailure: onGetSurveyFailure,
